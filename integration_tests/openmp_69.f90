program test_schedule_comparison
    use omp_lib
    implicit none
    ...
    if (static_time < dynamic_time * 0.9 .and. static_time < guided_time * 0.9) then
        print *, "✓ STATIC is fastest for uniform workload (expected)"
    else if (dynamic_time < static_time * 0.9) then
        print *, "! DYNAMIC is fastest (unexpected for uniform work)"
    else if (guided_time < static_time * 0.9) then
        print *, "! GUIDED is fastest (unexpected for uniform work)"
    else
    ...
    ! For non-uniform workload, dynamic should perform better
    if (dynamic_time < static_time * 0.95) then
        print *, "✓ DYNAMIC performs better for non-uniform workload (expected)"
    else
        print *, "! DYNAMIC did not show expected improvement"
    end if
    
end program test_schedule_comparison